;; Copyright (c) 2012 The Chromium Authors & Jamie Kirkpatrick.
;; Use of this source code is governed by a BSD-style license that can be
;; found in the LICENSE file.

(require 'flymake)

(defcustom ninjafly-ninja-build-files '("out/Debug/build.ninja")
  "Relative path from the project root to the build.ninja file to use.")

(defcustom ninjafly-ninja-executable "ninja"
  "Ninja executable location; either in $PATH or explicitly given.")

(defun ninjafly-ninja-build-file ()
  "Return the build file path that is in use for the current project
   or nil if the current buffer does not have a matching file"
  (loop for file in ninjafly-ninja-build-files do
        (let ((srcdir (locate-dominating-file
                       (ninjafly-absbufferpath) file)))
          (when srcdir
            (return file)))))

(defun ninjafly-absbufferpath ()
  "Return the absolute path to the current buffer, or nil if the
  current buffer has no path."
  (when buffer-file-truename
      (expand-file-name buffer-file-truename)))

(defun* ninjafly-project-dir ()
  "Return the project directory, or nil on failure."
  (if (ninjafly-ninja-build-file)
      (let ((srcdir (locate-dominating-file
                     (ninjafly-absbufferpath) (ninjafly-ninja-build-file))))
        (when srcdir (expand-file-name srcdir)))))

(defun ninjafly-string-prefix-p (prefix str)
  "Return non-nil if PREFIX is a prefix of STR (23.2 has string-prefix-p but
  that's case insensitive and also 23.1 doesn't have it)."
  (string= prefix (substring str 0 (length prefix))))

(defun ninjafly-current-file-name ()
  "Return the relative path from chromium's src/ directory to the
  file backing the current buffer or nil if it doesn't look like
  we're under chromium/src/."
  (when (and (ninjafly-project-dir)
             (ninjafly-string-prefix-p
              (ninjafly-project-dir) (ninjafly-absbufferpath)))
    (substring (ninjafly-absbufferpath) (length (ninjafly-project-dir)))))

(defun ninjafly-from-build-to-src-root ()
  "Return a path fragment for getting from the build.ninja file to src/."
  (replace-regexp-in-string
   "[^/]+" ".."
   (substring
    (file-name-directory
     (file-truename (or (and (ninjafly-string-prefix-p
                              "/" (ninjafly-ninja-build-file))
                             (ninjafly-ninja-build-file))
                        (concat (ninjafly-project-dir)
                                (ninjafly-ninja-build-file)))))
    (length (ninjafly-project-dir)))))

(defun ninjafly-getfname (file-name-from-error-message)
  "Strip cruft from the passed-in filename to help flymake find the real file."
  (file-name-nondirectory file-name-from-error-message))

(defun ninjafly-ninja-command-line ()
  "Return the command-line for running ninja, as a list of strings, or nil if
  we're not during a save"
  (unless (buffer-modified-p)
    (list ninjafly-ninja-executable
          (list "-C"
                (concat (ninjafly-project-dir)
                        (file-name-directory (ninjafly-ninja-build-file)))
                (concat (ninjafly-from-build-to-src-root)
                        (ninjafly-current-file-name) "^")))))

(defun ninjafly-kick-off-check-after-save ()
  "Kick off a syntax check after file save, if flymake-mode is on."
  (when flymake-mode (flymake-start-syntax-check)))

(defadvice next-error (around ninjafly-next-error activate)
  "If flymake has something to say, let it say it; otherwise
   revert to normal next-error behavior."
  (if (not flymake-err-info)
      (condition-case msg
          ad-do-it
        (error (message "%s" (prin1-to-string msg))))
    (flymake-goto-next-error)
    ;; copy/pasted from flymake-display-err-menu-for-current-line because I
    ;; couldn't find a way to have it tell me what the relevant error for this
    ;; line was in a single call:
    (let* ((line-no (flymake-current-line-no))
           (line-err-info-list
            (nth 0 (flymake-find-err-info flymake-err-info line-no)))
           (menu-data (flymake-make-err-menu-data line-no line-err-info-list)))
      (prin1 (car (car (car (cdr menu-data)))) t))))

(defun ninjafly-find-file ()
  "Enable flymake, but only if it makes sense, and immediately
  disable timer-based execution."
  (when (and (not flymake-mode)
             (not buffer-read-only)
             (ninjafly-current-file-name))
    ;; Since flymake-allowed-file-name-masks requires static regexps to match
    ;; against, can't use ninjafly-project-dir here.  Instead we add a
    ;; generic regexp, but only to a buffer-local version of the variable.
    (set (make-local-variable 'flymake-allowed-file-name-masks)
         (list (list "\\.c\\(\\|c\\|pp\\)"
                     'ninjafly-ninja-command-line
                     'ignore
                     'ninjafly-getfname)))
    (flymake-find-file-hook)
    (if flymake-mode
        (cancel-timer flymake-timer)
      (kill-local-variable 'flymake-allowed-file-name-masks))))

(add-hook 'find-file-hook 'ninjafly-find-file 'append)
(add-hook 'after-save-hook 'ninjafly-kick-off-check-after-save)

;; Show flymake infrastructure ERRORs in hopes of fixing them.  Set to 3 for
;; DEBUG-level output from flymake.el.
(setq flymake-log-level 0)

(provide 'ninjafly)
